class Registry < ApplicationRecord
  ...
  def update_extra_counts
    ...
    self.downloads = packages.where(status: 'active').or(packages.where.not(status: ['removed', 'unpublished'])).sum(:downloads)
    self.dependent_repos_count = packages.where(status: 'active').or(packages.where.not(status: ['removed', 'unpublished'])).sum(:dependent_repos_count)
    save
  end
  ...
  def keywords
    Rails.cache.fetch("registries_keywords/#{id}", expires_in: 1.week) do
      result = Package.connection.select_rows("SELECT keywords, COUNT(keywords) AS keywords_count FROM (SELECT id, registry_id, unnest(keywords) AS keywords FROM packages WHERE registry_id = #{id} AND status IS NULL) AS foo GROUP BY keywords ORDER BY keywords_count DESC, keywords ASC LIMIT 50000;")
      result || []
    end
  end
  ...
  def least_recently_synced_package
    return nil if least_recently_synced_package_id.nil?
  ...
  def one_percent_of_packages_count
    ...
    return 4000 if count > 4000
    return 300 if count < 200
    count
  end
